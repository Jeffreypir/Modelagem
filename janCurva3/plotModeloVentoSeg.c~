...
int main(void){
	...
	fprintf(gp,"set title 'Curva do vento' \n");
	fprintf(gp,"set output 'ventoSeg.pdf'\n");
	fprintf(gp,"set key right bottom \n");
	fprintf(gp,"set key font 'Times,10' \n");
	fprintf(gp,"set origin 0,0 \n");
	fprintf(gp,"set xlabel 'Tempo (s)'\n");
	fprintf(gp,"set ylabel 'Velociadade (m/s)' \n");
	fprintf(gp,"set grid back ls 10\n");
	fprintf(gp,"plot 'ventoSeg.txt' using 1:2 with points lc 6 pt 7 ps 0.1 t'Vento'\n");
    fclose(gp);
